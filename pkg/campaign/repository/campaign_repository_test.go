...
func TestDraftValidationBoundsContentAndAttribution(t *testing.T) {
	tests := []struct {
		...
		{name: "image without media", input: &DraftInput{Name: "name", ContentType: campaign_model.CampaignContentImage, Recipients: validRecipientConsent(), Actor: Actor{Type: "system"}}},
		{name: "oversized image caption", input: &DraftInput{Name: "name", ContentType: campaign_model.CampaignContentImage, MediaAssetID: uuid.NewString(), TextBody: strings.Repeat("x", 1025), Recipients: validRecipientConsent(), Actor: Actor{Type: "system"}}},
		{name: "text with media", input: &DraftInput{Name: "name", ContentType: campaign_model.CampaignContentText, TextBody: "hello", MediaAssetID: uuid.NewString(), Recipients: validRecipientConsent(), Actor: Actor{Type: "system"}}},
