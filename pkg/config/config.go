...
func Load() *Config {
	...
	if canonicalConversationIdentityEnabled && !contactIdentityReconciliationEnabled {
		logger.LogFatal("[CONFIG] %s requires %s=true", config_env.WA_CANONICAL_CONVERSATION_IDENTITY_ENABLED, config_env.WA_CONTACT_IDENTITY_RECONCILIATION_ENABLED)
	}
	if (conversationAppStateCommandsEnabled || conversationHistorySyncEnabled) && !canonicalConversationIdentityEnabled {
		logger.LogFatal("[CONFIG] Conversation commands require %s=true", config_env.WA_CANONICAL_CONVERSATION_IDENTITY_ENABLED)
	}
	...
	legacyGroupMyAllEnabled, err := parseOptionalStrictBool(defaultIfEmpty(strings.TrimSpace(os.Getenv(config_env.WA_LEGACY_GROUP_MYALL_ENABLED)), "true"))
	if err != nil {
		logger.LogFatal("[CONFIG] invalid %s: %v", config_env.WA_LEGACY_GROUP_MYALL_ENABLED, err)
	}
	...
	config := &Config{
		...
		LegacyGroupMyAllEnabled:       legacyGroupMyAllEnabled,
