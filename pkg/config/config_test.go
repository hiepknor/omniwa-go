...
func TestLoadWAInfoGuardDefaults(t *testing.T) {
	...
	if len(config.HTTPAllowedOrigins) != 0 {
		t.Fatalf("HTTP allowed origin defaults = %v", config.HTTPAllowedOrigins)
	}
...
func TestLoadWAInfoGuardOverrides(t *testing.T) {
	...
	if len(config.HTTPAllowedOrigins) != 2 || config.HTTPAllowedOrigins[0] != "https://console.example.com" || config.HTTPAllowedOrigins[1] != "https://admin.example.com/" {
		t.Fatalf("HTTP allowed origin overrides = %v", config.HTTPAllowedOrigins)
	}
...
func TestLoadIgnoresRetiredChatNamedConversationIdentitySetting(t *testing.T) {
	setRequiredConfigEnv(t)
	t.Setenv(config_env.WA_CONTACT_IDENTITY_RECONCILIATION_ENABLED, "true")
	t.Setenv(config_env.WA_CANONICAL_CONVERSATION_IDENTITY_ENABLED, "")
	t.Setenv("WA_CANONICAL_CHAT_IDENTITY_ENABLED", "true")
	...
	if loaded.CanonicalConversationIdentityEnabled {
		t.Fatal("retired Chat-named setting unexpectedly enabled canonical Conversation identity")
	}
}
