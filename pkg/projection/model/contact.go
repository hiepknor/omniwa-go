...
type Contact struct {
	...
	Aliases                  []ContactIdentity `json:"-" gorm:"-"`
}

func (Contact) TableName() string { return "projected_contacts" }

func (contact *Contact) AddressingJID() string {
	if contact == nil {
		return ""
	}
	...
	return contact.PreferredJID
}

func (contact *Contact) CanonicalDisplayName() (string, string) {
	if contact == nil {
		return "", ""
	}
	for _, candidate := range []struct {
		...
		{contact.FullName, "full_name"}, {contact.BusinessName, "business_name"},
		{contact.PushName, "push_name"}, {contact.FirstName, "first_name"}, {contact.Username, "username"},
	} {
